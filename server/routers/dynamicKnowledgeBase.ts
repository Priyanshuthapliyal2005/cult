...
import { mockChromeDb } from '@/lib/mock-chromadb';
...
export const dynamicKnowledgeBaseRouter = router({
  ...
  testConnection: publicProcedure
    .query(async () => {
      return await mockChromeDb.testConnection();
    }),
  ...
  listCollections: publicProcedure
    .query(async () => {
      return await mockChromeDb.listCollections();
    }),
  ...
  getCollectionStats: publicProcedure
    ...
    .query(async ({ input }) => {
      return await mockChromeDb.getCollectionStats(input.name);
    })
});